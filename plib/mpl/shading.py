...
def set_shade(a, intensity=None, cmap=cm.jet, scale=10.0, azdeg=270.0, altdeg=55.0,
              vmin=None, vmax=None):
...
def shadow_from_alpha(rgba, sigma=20, r=0.5, b=0.5, g=0.5):
    from scipy.ndimage.filters import gaussian_filter
    r_shadow = np.ones(rgba[:, :, 0].shape) * r
    g_shadow = np.ones(rgba[:, :, 0].shape) * g
    b_shadow = np.ones(rgba[:, :, 0].shape) * b
    a_shadow = gaussian_filter(rgba[:, :, 3], sigma=sigma)
    rgba_shadow = np.dstack((r_shadow, g_shadow, b_shadow, a_shadow)).copy()
    return rgba_shadow
